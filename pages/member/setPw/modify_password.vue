<template>
	<view class="flex align-center bg-yeq-page" style="height: 100vh;">
		<cu-custom bgColor="bg_black text-white" :isBack="true"><block slot="backText"></block><block slot="content">修改登录密码</block></cu-custom>
		...
		<view class="margin-lr-lg flex-sub padding-tb-lg bg-white radius box-shadow">
			...
			<view class="height80 margin-lr-sm margin-top-sm solid_1 radius flex align-center justify-between">
				...
				<view class="bg-red height80 flex text-df align-center padding-lr-xs checking bg-lan" @click="checking" v-if="state===false">获取验证码</view>
				<view class="bg-disabled height80 text-df flex align-center padding-lr-xs checking" v-if="state===true">倒计时{{ currentTime }}s</view>
			</view>
			<button class="add-btn height80 bg-red margin-top-lg margin-lr-sm flex align-center justify-center text-lg bg-lan" @click="submit">提交</button>
